<x-filament-widgets::widget>
    <x-filament::section>
        <x-slot name="heading">Peta sebaran lokasi terlapor</x-slot>
        <x-slot name="description">Titik menunjukkan lokasi tempat atau kegiatan yang dilaporkan masyarakat, bukan lokasi pelapor.</x-slot>
        ...
        <div wire:ignore>
            <div id="admin-report-map-{{ $this->getId() }}" class="w-full overflow-hidden rounded-xl bg-gray-100" style="height: 460px; width: 100%;"></div>
            <div class="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                <span class="inline-flex items-center gap-2"><span class="size-2.5 rounded-full bg-amber-500"></span>Dalam penanganan</span>
    ...
    @script
        <script>
            (() => {
                ...
                const map = L.map(element, { minZoom: 7, maxZoom: 18, scrollWheelZoom: false }).setView([-8.72, 117.35], 8);
